...
Claude Code hooks 覆盖全部阶段（Phase 00-07），利用其 hooks 机制对 AI 的实时工具调用做程序化检查。Phase 00-02（人工介入期）有额外的阶段推进拦截和 Stop 门禁；CC-8 对 Requirements + AI 自动期（Phase 02-06）做阶段出口门禁。
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash $CLAUDE_PROJECT_DIR/scripts/cc-hooks/pre-write-dispatcher.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash $CLAUDE_PROJECT_DIR/scripts/cc-hooks/doc-structure-check.sh"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash $CLAUDE_PROJECT_DIR/scripts/cc-hooks/read-tracker.sh"
          }
        ]
      }
    ],
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash $CLAUDE_PROJECT_DIR/scripts/cc-hooks/stop-gate.sh"
          }
        ]
      }
    ],
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash $CLAUDE_PROJECT_DIR/scripts/cc-hooks/inject-phase-context.sh"
          }
        ]
      }
    ]
...
{
  "decision": "block",
  "reason": "当前处于人工介入期（$PHASE），禁止 AI 自行推进阶段。请等待用户明确确认后再更新「当前阶段」。"
}
EOF
  exit 0
fi
...
**行为**：返回 JSON block 阻止工具调用；AI 仍可正常编辑 status.md 的其他字段（如设置运行状态为 `wait_confirm`）。
...
# 注：代码文件（如 src/、app/）不在该 hook 职责范围，由常规代码审查与 Git 门禁兜底。
...
case "$PHASE" in
  ...
  Implementation)
    ...
    # spotcheck_ 为人类抽检文件；implementation_checklist.md 为可选自检留痕
...
{
  "decision": "block",
  "reason": "当前阶段 $PHASE，不允许创建/修改 $BASENAME。本阶段允许的文件：${ALLOWED//|/, }"
}
EOF
  exit 0
fi
...
# 阶段入口必读清单（单源）
ENTRY_REQUIRED=$(aicoding_phase_entry_required "$PHASE" "$VERSION_DIR" | tr '\n' ',' | sed 's/,$//')
...
{
  "decision": "block",
  "reason": "review 文件已有 ${EXISTING_ROUNDS} 轮审查记录，新内容只有 ${NEW_ROUNDS} 轮。审查记录必须追加，不得覆盖。请使用 Edit 工具在文件末尾追加新一轮审查。"
}
EOF
  exit 0
fi
...
**逻辑（当前实现，节选自 `pre-write-dispatcher.sh` Gate 4）**：
```bash
# 1) 只在首次写入阶段产出物时触发；review/status 文件跳过
# 2) hotfix（_change_level=hotfix）跳过入口门禁
# 3) 必读清单来自单源函数 aicoding_phase_entry_required
# 4) 按 aicoding.config.yaml 的 entry_gate_mode=warn|block 执行
...
if [ -n "$MISSING" ]; then
  WARN_MSG="阶段入口门禁（${AICODING_PHASE}）：写入产出物前必须先读取以下文件：..."
...
**行为**：`entry_gate_mode=block` 时返回 JSON block 阻断写入；`warn` 时仅告警并记录 warning 日志，不阻断。
...
**逻辑（当前实现，节选自 `pre-write-dispatcher.sh` Gate 5）**：
```bash
# 1) 仅当 status.md 发生阶段推进时触发
# 2) 生效阶段：Requirements/Design/Planning/Implementation/Testing（Phase 02-06）
# 3) 必要产出物清单来自单源函数 aicoding_phase_exit_required
# 4) minor 分支独立处理：Implementation/Testing 用 review_minor.md
# 5) Testing+minor 额外要求：
#    - 测试证据（test_report.md 或 status.md TEST-RESULT）
#    - review_minor.md 包含 MINOR-TESTING-ROUND 机器可读块
...
if [ "$AICODING_PHASE" = "Testing" ] && [ "$CHANGE_LEVEL" = "minor" ]; then
  _has_minor_test_evidence "${VERSION_PATH}status.md" || aicoding_block "Testing minor 缺少测试证据"
...
**行为**：通过 `aicoding_block` 返回 JSON block 阻止 `_phase` 推进，并携带缺失项/校验失败原因。
...
| CC-8 | 阶段出口门禁 | PreToolUse | 流程守卫 | Phase 02-06 | AI 产出物不完整就推进阶段 |
...
commit 时
  ...
  │   失败 → 阻止 commit
  │
  ├─ post-commit（软警告，21 个脚本 + 内置 W23/W24）
  │   有价值但可能存在边界情况的检查
...
### Git-Hook 1：commit-msg 前缀与单版本策略校验

| 项 | 值 |
|---|---|
| 触发点 | `commit-msg` |
| 对应规则 | STRUCTURE.md Git 规范 + `ai_workflow.md` 单版本最新策略 |
| 误报率 | 极低 |
...
**补充约束**：同一提交不得同时暂存多个 `docs/vX.Y/` 版本目录；命中多个版本时，`commit-msg` 会直接阻断提交，强制回到单版本提交策略。
...
for f in $STATUS_FILES; do
  ...
  BASELINE=$(echo "$FRONT_MATTER" | grep '^_baseline:' | sed 's/^_baseline:[[:space:]]*//' | tr -d '[:space:]')
  ...
  # _change_level 兼容期：缺失仅告警，存在时必须 major/minor/hotfix
  if [ -n "$CHANGE_LEVEL" ]; then
    case "$CHANGE_LEVEL" in major|minor|hotfix) ;; *)
      echo "❌ $f: _change_level 值非法（兼容期仅允许 major/minor/hotfix）"; exit 1 ;; esac
  fi
  ...
  echo "$BASELINE" | grep -qE '^v[0-9]+\.[0-9]+$' || {
    echo "❌ $f: _baseline 必须为可识别版本 tag（格式 vX.Y）"; exit 1
  }
done
```
...
|---|---|
| 触发点 | `pre-commit` |
| 对应规则 | `05-implementation.md` / `06-testing.md` / `07-deployment.md` 交付规则 |
| 触发条件 | 暂存区包含 `status.md`，且 `_phase` 为 `Testing/Deployment` 并进入 `wait_confirm` / `done` / `completed` 交付信号 |
| 误报率 | 低（仅关键节点触发） |

**语义**：不强跑测试/scan，只做确定性检查：交付前必须有可复现证据、可判定结论，以及与当前状态机一致的交付文档。
...
- **Deployment 专项硬拦截**：当 `_phase=Deployment` 时，额外要求：
  - `deployment.md` 必须存在
  - `deployment.md` 中的目标环境必须合法
  - `deployment.md` 记录的“本次上线 CR 列表”必须是 `status.md` Active CR 的子集
  - 只有在 `deployment.md` 明确记录“验收结论：通过”后，才允许进入 `done/completed`
- **分支补充**：`minor` 交付额外校验 `review_minor.md` 与 Testing 轮次块；`hotfix` 完成态额外要求 `status.md` 内联 `TEST-RESULT` 证据

---

### Git-Hook 7：阶段出口门禁（阶段推进产出物检查）

| 项 | 值 |
|---|---|
| 触发点 | `pre-commit` |
| 对应规则 | CC-8 出口门禁的 Git 层等价物 |
| 触发条件 | 暂存区 status.md 的 `_phase` 与 HEAD 不同（检测到阶段推进） |
| 误报率 | 极低（仅阶段推进时触发） |

> 说明：**新建** `status.md` 不属于“阶段推进”路径，初始化规则单独处理：版本内 CR 必须从 `ChangeManagement` 开始；新版本启动允许在 `Proposal + 无 Active CR` 时直接开始。

**语义**：当 `_phase` 发生阶段推进时，检查旧阶段的必要产出物是否存在。覆盖 ChangeManagement/Proposal/Requirements/Design/Planning/Implementation/Testing。与 CC-8 逻辑一致，但在 Git 层拦截，对 Claude Code 和 Codex 都生效。
...
| 离开阶段 | 必须存在的文件 |
|---------|--------------|
| ChangeManagement | `review_change_management.md` |
| Proposal | `review_proposal.md` |
| Requirements | `review_requirements.md` |
| Design | `design.md`, `review_design.md` |
| Planning | `plan.md`, `review_planning.md` |
| Implementation | major: `review_implementation.md`; minor: `review_minor.md` |
| Testing | major: `test_report.md`, `review_testing.md`; minor: `review_minor.md` + 测试证据（`test_report.md` 或 status 内联 `TEST-RESULT`） |
...
for f in $STATUS_FILES; do
  ...
  CHANGE_LEVEL=$(echo "$NEW_FM" | grep '^_change_level:' | sed 's/^_change_level:[[:space:]]*//' | tr -d '[:space:]')
  [ -z "$CHANGE_LEVEL" ] && CHANGE_LEVEL="major"
  if [ "$CHANGE_LEVEL" = "hotfix" ]; then
    echo "❌ ${f}: hotfix 不允许推进阶段（${OLD_PHASE} → ${NEW_PHASE}）"
    exit 1
  fi
  ...
  while IFS= read -r required_doc; do
    ...
    check_exit_artifact "${VERSION_DIR}${required_doc}"
  done < <(aicoding_phase_exit_required "$OLD_PHASE" "$CHANGE_LEVEL")

  if [ "$OLD_PHASE" = "Testing" ] && [ "$CHANGE_LEVEL" = "minor" ]; then
    has_test_report=true
    git cat-file -e ":${VERSION_DIR}test_report.md" 2>/dev/null || has_test_report=false
    has_inline_test_result=$(git show ":$f" 2>/dev/null | grep -c 'TEST-RESULT-BEGIN' || true)
    if [ "$has_test_report" = false ] && [ "${has_inline_test_result:-0}" -eq 0 ]; then
...
> **`--no-verify` 与 post-commit**：`--no-verify` 只跳过 pre-commit 和 commit-msg，**不跳过 post-commit**。即使 AI 使用了 `--no-verify` 绕过硬拦截，所有软警告仍然会正常执行并输出到终端。

**完整清单**：`warnings/` 目录下 21 个脚本（W6–W22, W25–W28）+ post-commit dispatcher 内置 2 个检查（W23 审查轮次告警、W24 逃生通道审计）。本节详细记录 W6–W18 的伪代码；W19–W22、W25–W28 的实现见 `scripts/git-hooks/warnings/` 对应脚本文件。
...
> **口径更新**：Minor 不再使用 `[Minor]` 文本标记；统一以 `_change_level: minor` 为准。Requirements 阶段仍要求 `proposal.md` 作为输入，不再提供跳过豁免。
...
### Warning 10：基线版本 tag 可达性

| 项 | 值 |
|---|---|
| 对应规则 | 00-change-management.md 基线验证（`_baseline` 必须为版本 tag，如 `v1.0`） |
| 触发条件 | 本次 commit 包含 `status.md` 变更 |
...
echo "$BASELINE" | grep -qE '^v[0-9]+\.[0-9]+$' || \
  warn "基线版本 $BASELINE 格式非法（期望版本 tag，如 v1.0）"
...
| 项 | 值 |
|---|---|
| 对应规则 | CR 影响面与 diff 一致性检查（原 review_template.md AC-05，附录已移除） |
...
| 项 | 值 |
|---|---|
| 对应规则 | Deployment 审计提醒（非主门禁） |
| 触发条件 | 当前阶段 = Deployment |

> 说明：这条 warning 只检查 `baseline..HEAD` 的 commit message 是否覆盖 Active CR，帮助补审计线索；它**不替代** `pre-commit` 对 `deployment.md`“本次上线 CR 列表 ⊆ Active CR” 的硬拦截。
...
| 项 | 值 |
|---|---|
| 对应规则 | 流程增强：审计与证据补强（原 enhance.md 落地项 B，已合并入框架） |
...
| 项 | 值 |
|---|---|
| 对应规则 | 流程增强：验收命令强化（原 enhance.md 落地项 C，已合并入框架） |
...
| 项 | 值 |
|---|---|
| 对应规则 | 流程增强：高风险触发器（原 enhance.md 落地项 D，已合并入框架） |
...
| # | 副作用 | 严重度 | 应对措施 |
|---|--------|--------|---------|
| 1 | **误报累积** | 中 | 硬拦截 7 个（含交付关口条件拦截+出口门禁）只卡确定性高的缺口；软警告（21 脚本 + 内置 W23/W24）不阻断，误报可忽略 |
...
Claude Code hooks 通过 `.claude/settings.local.json`（或 `.claude/settings.json`）配置，无需安装脚本。配置内容见 §4.2。

> 注意：Claude Code 在启动时快照 hooks 配置。修改 settings 后需通过 `/hooks` 菜单确认变更生效。
...
│   └── cc-hooks/
│       ├── pre-write-dispatcher.sh  ← PreToolUse 统一入口（合并 CC-1/3/6/7/8）
│       ├── phase-gate.sh            ← CC-Hook 1：阶段推进拦截（逻辑子模块）
│       ├── stop-gate.sh             ← CC-Hook 2：Stop 门禁
│       ├── doc-scope-guard.sh       ← CC-Hook 3：文档作用域控制（逻辑子模块）
│       ├── inject-phase-context.sh  ← CC-Hook 4：会话上下文注入（含入口必读清单）
│       ├── doc-structure-check.sh   ← CC-Hook 5：产出物结构校验
│       ├── review-append-guard.sh   ← CC-Hook 6：review 追加保护（逻辑子模块）
│       ├── phase-entry-gate.sh      ← CC-Hook 7：阶段入口门禁（逻辑子模块）
│       ├── read-tracker.sh          ← CC-Hook 7b：Read 追踪器
│       └── phase-exit-gate.sh       ← CC-Hook 8：阶段出口门禁（逻辑子模块）
├── ai_workflow.md
├── hooks.md                         ← 本文件（方案说明）
└── AGENTS.md.template
...
| `review_template.md` | CR 影响面一致性检查 → Warning 12 | CC-6 保护 review 文件追加完整性；CC-2 确保审查文件存在；CC-8 确保 review 文件存在才能推进 |
...
| 2026-02-10 | 硬拦截 7 个（含交付关口条件拦截+出口门禁）+ 软警告（21 脚本 + 内置 W23/W24）分层 | 确定性高的检查硬拦截；有边界情况的检查移入软警告；交付关口补充条件拦截减少漏检 |
...
| 2026-02-10 | PreToolUse/PostToolUse/Stop 统一 JSON block 协议 | 通过 `aicoding_block` 单点实现阻断输出，避免不同脚本的阻断格式漂移 |
...
| 2026-02-12 | CC-8 阶段出口门禁在 Phase 02-06 生效 | Phase 00-01 由人工确认把关；Phase 07 由交付关口条件拦截 + 部署流程规则兜底 |
| 2026-03-06 | Deployment 阶段统一解释 `wait_confirm` 语义 | 对 STAGING/TEST 表示“已部署，等待业务反馈”，不表示部署前批准 |
