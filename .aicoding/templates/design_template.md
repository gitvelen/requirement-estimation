...
| ... |  |  |  |  |
<!-- 敏感项实际值写入.env不进git；留空项标记"待部署阶段确认"，Deployment阶段提醒补填 -->
## 1. 背景、目标、非目标与约束
### 1.1 背景与问题
- 现状与痛点：/ 触发原因：/ 相关历史：
### 1.2 目标（Goals，可验收）
...
| 维度 | DEV | STAGING | PROD |
|------|-----|---------|------|
| 数据 / 外部依赖 | mock/seed / mock/stub | 脱敏副本 / 沙箱 | 真实数据 / 真实服务 |
| 配置来源 / 密钥管理 | .env.local / 本地文件 | 配置中心(staging) / Vault(staging) | 配置中心(prod) / Vault(prod) |
| 网络策略 / 资源规格 / 监控告警 | 无限制 / 最小化 / 可选 | 接近生产 / 接近生产 / 开启(不通知) | 生产策略 / 生产规格 / 开启(通知) |
