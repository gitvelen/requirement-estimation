...
> 默认不启用：仅当用户明确触发（推荐触发词：`@review`）时，Claude/Codex 才进入“Reviewer 模式”。  
> 目标：以**建议性质**的方式，审查阶段产出/代码变更，给出可执行建议与可复现证据，并将报告**落盘**（若有更新，则在文末追加，并且标明追加的版本号）。
...
| **P1（Major）** | 功能降级/边界未处理/性能不达标/文档与代码不一致/回滚方案缺失/CR 影响面遗漏 | 必须修复或明确 accept/defer（写清理由+缓解） | Fix / Accept / Defer |
