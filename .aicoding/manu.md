...
## 总览：你需要做什么 vs AI 帮你做什么

整个项目建设分为 **8 个阶段**，按人工介入程度分为三个时期：

> **提示**：如果是小改动（单模块 Bug 修复、UI 微调等），你可以告诉 AI "走简化流程"，AI 会合并部分阶段但仍确保需求理解到位。
...
| 异常 | 触发条件 | 你需要做什么 |
|------|---------|-------------|
| **3 轮不收敛** | AI 自动期（阶段 3-6）连续 3 轮自我审查仍有 P0/P1 问题 | 选择：(1) 标记部分 P1 为 accept/defer (2) 给反馈 (3) 跳过阶段 |
...
**你只需要在前两个阶段（提案+需求）深度参与决策，中间四个阶段（设计→测试）AI 全自动完成，最后部署阶段你做最终确认。** 整个过程中，AI 负责所有文档生成、审查、门禁验证和状态管理，你只需要在关键节点做决策和确认。
