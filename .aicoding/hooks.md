...
| 约束 | 说明 |
|------|------|
| 双工具兼容（AI 自动期） | Phase 03-06 同时使用 Claude Code 和 Codex，hooks 必须对两者都生效 |
...
| 人工介入期锁定 Claude Code | Phase 00-02 确定使用 Claude Code，可使用 Claude Code hooks 做更细粒度的控制 |
...
用户指挥多个 AI 工具（多个 Claude、Codex 等）协作开发，无 CI 环境。根据阶段特征采用两层互补的 hooks 体系：
...
│                                                                  │
│  拦截 commit 动作                                                │
│  硬拦截 6 个（pre-commit / commit-msg，其中含交付关口条件拦截）   │
│  软警告 13 个（post-commit）                                     │
...
**为什么不用 CI**：当前无 PR 流程，多个 AI 工具均在本地执行，引入 CI 增加维护成本但收益有限。

**未来扩展**：如果团队扩大或引入 CI/PR 流程，post-commit 软警告可直接升级为 CI 硬门禁，架构无需重构。

> **前置动作（🔴 MUST）**：在 AGENTS.md.template 的"Git 授权边界"中补充：`--no-verify` 必须用户明确授权，AI 不得自行使用。本方案依赖该文本层约束。

---

## 4. Claude Code Hooks：人工介入期守卫（6 个）

Phase 00-02 确定使用 Claude Code，可利用其 hooks 机制对 AI 的实时工具调用做程序化检查。
...
**解决的问题**：AI 在 Proposal 阶段就开始写 design.md 或 plan.md 等后续阶段产出物。比 Git Warning 6 更强——事前拦截而非事后警告。
...
**解决的问题**：新会话或 resume 时 AI 不知道当前阶段，盲目开始工作。
...
**行为**：SessionStart 的 additionalContext 会被注入到 AI 的上下文中，AI 在会话开始时就知道当前状态。零副作用。
...
| # | 名称 | 事件 | 层级 | 解决的问题 |
|---|------|------|------|-----------|
| CC-1 | 阶段推进拦截 | PreToolUse | 流程守卫 | AI 自行推进阶段 |
| CC-2 | Stop 门禁 | Stop | 流程守卫 | AI 未设 wait_confirm 或未走审查就停止 |
| CC-3 | 文档作用域控制 | PreToolUse | 流程守卫 | AI 越阶段创建产出物 |
| CC-4 | 会话上下文注入 | SessionStart | 流程守卫 | 新会话不知道当前阶段 |
| CC-5 | 产出物结构校验 | PostToolUse | 产出物校验 | 缺少必填章节、缺 GWT、CR 影响面未填 |
| CC-6 | review 追加保护 | PreToolUse | 产出物校验 | 覆盖已有审查记录 |

---

## 5. Git Hooks：硬拦截（6 个，其中 2 个为交付关口条件拦截）
...
commit 时
  │
  ├─ pre-commit / commit-msg（硬拦截，6 个）
  │   确定性高、误报率极低的检查
  │   失败 → 阻止 commit
  │
  ├─ post-commit（软警告，13 个）
  │   有价值但可能存在边界情况的检查
...
| 触发条件 | status.md 存在 Active CR（状态为 Accepted/In Progress） |
...
for f in $STATUS_FILES; do
  ...
  case "$PHASE" in Proposal|Requirements|Design|Planning|Implementation|Testing|Deployment) ;; *)
    echo "❌ $f: _phase 值非法（期望 Proposal/Requirements/Design/Planning/Implementation/Testing/Deployment）"; exit 1 ;; esac
done
```
...
## 6. Git Hooks：软警告（13 个）

post-commit 在 commit 完成后执行，不阻断工作流。输出警告信息，AI 看到后可自行修复并追加 commit。

> **`--no-verify` 与 post-commit**：`--no-verify` 只跳过 pre-commit 和 commit-msg，**不跳过 post-commit**。即使 AI 使用了 `--no-verify` 绕过硬拦截，13 个软警告仍然会正常执行并输出到终端。
...
| # | 副作用 | 严重度 | 应对措施 |
|---|--------|--------|---------|
| 1 | **误报累积** | 中 | 硬拦截 6 个（含交付关口条件拦截）只卡确定性高的缺口；软警告 13 个不阻断，误报可忽略 |
...
| 想法 | 排除理由 |
|------|---------|
| **先读后写（R7）** | 需要跨工具调用维护"已读文件"状态（临时文件），跨会话易失效；Bash 直接读取不经过 hook；新建 vs 修改判断复杂。R7 仍通过 AGENTS.md 文本规则保障 |
| **检查 AI 是否读了模板** | 需要跟踪 Read 调用维护状态，不可靠。用 CC-5 检查产出物结构是更好的替代——不管读没读，写出来的东西必须对 |
...
| 批次 | hooks | 理由 |
|------|-------|------|
| **第一批** | CC-1（阶段推进拦截）+ CC-4（上下文注入） | 最核心（阶段门禁）+ 零副作用（信息注入），立即见效 |
| **第二批** | CC-2（Stop 门禁）+ CC-3（文档作用域） | 流程守卫完整化，观察是否有误拦截 |
| **第三批** | CC-5（结构校验）+ CC-6（review 追加保护） | 产出物校验，需根据实际模板微调检查项 |
...
│   ├── git-hooks/
│   │   ├── pre-commit               ← Git-Hook 3-6 dispatcher
...
│       ├── doc-scope-guard.sh       ← CC-Hook 3：文档作用域控制
│       ├── inject-phase-context.sh  ← CC-Hook 4：会话上下文注入
...
|---------|-----------------|------------------------|
| `ai_workflow.md` | commit 时验证阶段一致性 | 实时拦截阶段推进、Stop 门禁、上下文注入 |
| `phases/*.md` | 各阶段完成条件由 AI 自我审查执行，post-commit 做事后提醒 | 产出物结构校验（CC-5）确保必填章节存在 |
| `templates/*.md` | Warning 7 检查 CR 必填字段 | CC-5 检查 proposal/requirements/CR 结构完整性 |
| `lessons_learned.md` R1-R10 | R6（REQ 引用漂移）→ Warning 9；R7（先读后写）→ 排除；R8（TodoList）→ 不适合 hook | — |
| `AGENTS.md` | hooks 是 AGENTS.md 文本规则的程序化补充，不是替代 | 同左 |
| `review_template.md` | AC-05（diff-only 检查）→ Warning 12 | CC-6 保护 review 文件追加完整性；CC-2 确保审查文件存在 |
...
| 2026-02-10 | 硬拦截 6 个（含交付关口条件拦截）+ 软警告 13 个分层 | 确定性高的检查硬拦截；有边界情况的检查移入软警告；交付关口补充条件拦截减少漏检 |
