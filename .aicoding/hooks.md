...
2. 入口/出口的文件清单单源化（`scripts/lib/common.sh`），内容级校验集中在 hook / review 校验函数中。
...
> `wait_confirm` 与 `wait_feedback` 都需要按 `_phase + _run_status` 组合解释：前者用于 Phase 00-02 等人工介入期的“等待人工确认/决策”，后者用于 Deployment 阶段的“已部署，等待业务反馈”。
...
- 结果门禁（`result_gate_test/build/typecheck`，仅阶段推进 commit；未配置时降级为告警）
- requirements 结构完整性与引用一致性
- 交付关口条件校验（Deployment 目标环境 / 上线 CR 子集 / 完成态验收结论）

### 3.2 commit-msg（提交信息）
- 提交前缀格式校验（`feat/fix/docs/...`）
- 单版本提交限制（禁止一次提交同时触碰多个 `docs/vX.Y/`）
...
### 3.4 CC hooks（流程引导）
- `inject-phase-context.sh`：会话开始上下文注入（含入口必读提示）
- `pre-write-dispatcher.sh`：PreToolUse 统一入口（合并 phase-gate / doc-scope-guard / phase-exit-gate / review-append-guard）
- `doc-structure-check.sh`：PostToolUse（Write）文档结构校验
- `stop-gate.sh`：Stop 时人工介入期收口校验

## 4. 单源定义
- `scripts/lib/common.sh`
  - `aicoding_phase_entry_required`（文件级入口清单）
  - `aicoding_phase_exit_required`（文件级出口清单）
  - `aicoding_config_value` / `aicoding_yaml_value`
  - `aicoding_precommit_evidence_matches_current_head`
- `scripts/lib/validation.sh`
  - Minor review / testing round 内容级校验（供 pre-commit 和 pre-write-dispatcher 共用）
- `scripts/lib/review_gate_common.sh`
  - 内容级校验单源（GWT 覆盖、摘要块验真、追溯校验）

> 说明：入口必读现在只作为会话开始时的提示信息，不再尝试对 `Read` 行为做跨调用硬校验；结果约束由写入范围、阶段出口和 Git hooks 负责。
