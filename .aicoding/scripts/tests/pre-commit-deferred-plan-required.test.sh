...
VERIFICATION_COMMANDS: echo deferred-plan-check
REVIEW_RESULT: pass
<!-- REVIEW-SUMMARY-END -->
EOF

# 推进阶段：Implementation → Testing（DEFERRED 缺少计划应被拦截）
cat > docs/v1.0/status.md <<EOF
---
_baseline: v0.9
_current: ${BASE_SHA}
...
if bash scripts/git-hooks/pre-commit; then
  fail "expected pre-commit to block when DEFERRED_TO_STAGING lacks PLAN_ENV/PLAN_METHOD/PLAN_ETA"
fi
...
| GWT-REQ-001-07 | REQ-001 | ✅ | CODE_REF | \`src/example.ts:7\` |  |
| GWT-REQ-001-08 | REQ-001 | ✅ | CODE_REF | \`src/example.ts:8\` |  |
| GWT-REQ-001-09 | REQ-001 | DEFERRED_TO_STAGING | RUN_OUTPUT | \`echo deferred\` | PLAN_ENV=STAGING PLAN_METHOD=k6 PLAN_ETA=2026-02-20 |
| GWT-REQ-001-10 | REQ-001 | ✅ | CODE_REF | \`src/example.ts:10\` |  |
...
if ! bash scripts/git-hooks/pre-commit; then
  fail "expected pre-commit to allow DEFERRED_TO_STAGING when plan fields are present and within limit"
fi
