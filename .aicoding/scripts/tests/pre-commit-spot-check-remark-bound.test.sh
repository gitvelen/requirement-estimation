...
# 预期：至少 1 条风险/边界提示必须绑定到 SPOT_CHECK_GWTS 中的条目。
# 这里刻意把备注写在非 spot-check 的行上，spot-check 行备注为空。
cat > docs/v1.0/review_implementation.md <<EOF
## 2026-02-14 第1轮

| GWT-ID | REQ-ID | 判定 | 证据类型 | 证据（可复现） | 备注 |
|---|---|---|---|---|---|
| GWT-REQ-001-01 | REQ-001 | ✅ | CODE_REF | \`src/example.ts:1\` | 风险：仅对这条做了浅层静态验证 |
...
if bash scripts/git-hooks/pre-commit; then
  fail "expected pre-commit to reject: spot-check row has empty remark"
fi
