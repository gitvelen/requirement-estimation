...
|---|---|---|---|---|---|
| GWT-REQ-001-01 | REQ-001 | ✅ | RUN_OUTPUT | \`pytest -q\` 输出片段 | 风险：仅验证了最短路径 |
...
VERIFICATION_COMMANDS: echo test-report-evidence-na-rejected
REVIEW_RESULT: pass
<!-- REVIEW-SUMMARY-END -->
EOF
...
|--------|--------|---------|-------------------|---------|------|------|
| GWT-REQ-001-01 | REQ-001 | 示例 | TEST-001 | RUN_OUTPUT | N/A | ✅ |
...
if bash scripts/git-hooks/pre-commit; then
  fail "expected pre-commit to reject: test_report evidence must not be N/A"
fi
