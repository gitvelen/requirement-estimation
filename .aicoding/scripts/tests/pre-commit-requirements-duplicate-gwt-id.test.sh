...
- [ ] GWT-REQ-001-01: Given 条件，When 触发，Then 结果（重复）
EOF
...
VERIFICATION_COMMANDS: echo requirements-duplicate-gwt-id
REVIEW_RESULT: pass
<!-- REVIEW-SUMMARY-END -->
EOF

# 推进阶段：Implementation → Testing（requirements.md 重复 GWT-ID 应被拦截）
cat > docs/v1.0/status.md <<EOF
---
_baseline: v0.9
_current: ${BASE_SHA}
