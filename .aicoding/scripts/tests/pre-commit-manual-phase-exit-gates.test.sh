...
if bash scripts/git-hooks/pre-commit; then
  fail "expected ChangeManagement->Proposal to require review_change_management.md"
fi

cat > docs/v1.0/review_change_management.md <<'EOF_REVIEW'
## review change management
- ok
EOF_REVIEW

git add docs/v1.0/status.md docs/v1.0/review_change_management.md
if ! bash scripts/git-hooks/pre-commit; then
  fail "expected ChangeManagement->Proposal to pass with review_change_management.md"
fi

git commit -q -m "move to proposal"
...
git add docs/v1.0/status.md
if bash scripts/git-hooks/pre-commit; then
  fail "expected Proposal->Requirements to require review_proposal.md"
fi
...
if ! bash scripts/git-hooks/pre-commit; then
  fail "expected Proposal->Requirements to pass with review_proposal.md"
fi
