...
run_dispatcher() {
  ...
  output=$(echo "$input" | CLAUDE_PROJECT_DIR="$tmp_dir" bash scripts/cc-hooks/pre-write-dispatcher.sh 2>&1 || true)
  printf '%s\n' "$output"
}

assert_block_contains() {
  local output="$1" pattern="$2"
  echo "$output" | grep -q '"decision"[[:space:]]*:[[:space:]]*"block"' || fail "expected dispatcher to block, got: $output"
  echo "$output" | grep -q "$pattern" || fail "expected block message to contain [$pattern], got: $output"
}

output=$(run_dispatcher)
assert_block_contains "$output" "review_requirements.md"
...
output=$(run_dispatcher)
assert_block_contains "$output" "Requirements 禁止项确认校验失败"
...
output=$(run_dispatcher)
assert_block_contains "$output" "Requirements Proposal 覆盖校验失败"
...
if echo "$output" | grep -q '"decision"[[:space:]]*:[[:space:]]*"block"'; then
  fail "expected dispatcher to allow Requirements->Design when all exit checks pass, got: $output"
fi
