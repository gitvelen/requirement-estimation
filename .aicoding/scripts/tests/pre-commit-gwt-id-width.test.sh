...
|---|---|---|---|---|---|
| GWT-REQ-100-01 | REQ-100 | ✅ | CODE_REF | \`src/example.ts:1\` | 风险：示例 |
...
GWT_FAIL: 0
GWT_WARN: 0
SPOT_CHECK_GWTS: GWT-REQ-100-01
VERIFICATION_COMMANDS: echo gwt-id-width
REVIEW_RESULT: pass
<!-- REVIEW-SUMMARY-END -->
EOF
...
if bash scripts/git-hooks/pre-commit; then
  fail "expected pre-commit to block because GWT-REQ-1000-01 is missing from review table"
fi
