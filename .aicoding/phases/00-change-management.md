...
当项目处于 `_change_status: done` + `_run_status: completed` 状态时，用户提出新变更需求，AI 必须：

1. **分析变更描述**，给出建议（参考下表"AI 建议参考"）
2. **询问用户确认**："这是 v1.0 的补丁修复，还是要开始 v1.1 新版本？"
3. **用户明确指定后**，执行对应流程
4. **所有变更都必须创建 CR**，进入 Phase 00 澄清流程

| 变更性质 | AI 建议参考（关键词） | 操作方式 | 版本号 | 基线 |
|---------|---------------------|---------|--------|------|
| 遗漏/Bug/Hotfix | "修复"、"Bug"、"遗漏"、"缺陷" | 在当前版本追加 CR | 不变（如 v1.0） | 不变（如 v1.0） |
| 新功能/重构/增强 | "新功能"、"新版本"、"重构"、"架构" | 创建新版本目录 | 递增（如 v1.1） | 指向前版本（v1.0） |
...
3. **用户确认后**，执行以下操作：
   ...
   - 创建 CR 文件：`docs/v<新版本>/cr/CR-YYYYMMDD-001.md`
   - CR 字段填写：
     - 基线版本：前一版本 tag（如 `v1.0`）
     - 目标版本：新版本（如 `v1.1`）
4. **进入 Phase 00 澄清流程**
