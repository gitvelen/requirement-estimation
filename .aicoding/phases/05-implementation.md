...
- `docs/<版本号>/design.md`（major）
- `docs/<版本号>/requirements.md`
- `docs/<版本号>/status.md`
- `.aicoding/phases/05-implementation.md`（本文件）
- `.aicoding/templates/implementation_checklist_template.md`
- major: `.aicoding/templates/review_implementation_template.md`
- minor: `.aicoding/templates/review_minor_template.md`

**出口文件：**
- 代码改动
- `docs/<版本号>/implementation_checklist.md`（可选，建议用于自检留痕；允许产出但不属于出口门禁必需文件）
...
4. hotfix 不创建/推进 Implementation 阶段文档；它是在当前 `_phase` 冻结上下文内执行的紧急修复，仅适用 `ai_workflow.md` 中定义的 hotfix 边界与交付规则。
5. 如发现需求/设计冲突，先暂停并确认，不得自行改验收标准。
6. **版本重构功能对照清单（🔴 MUST）**：
   - 适用场景：Major 且涉及架构/路由/数据源重写
   - 清单路径：`docs/<版本号>/refactoring_checklist.md`
   - 清单模板：`.aicoding/templates/refactoring_checklist_template.md`
   - 必须在实现前产出清单，逐项验证后方可推进
   - 验证方式：运行时行为验证（禁止仅用静态字符串匹配）
   - 门禁检查：AI 自觉执行（如 `status.md` 标注"重构"且 `_phase >= Implementation`，必须产出清单）
...
**Implementation 阶段特有检查项**：
1. 确认本阶段非 review 产出物已存在（代码改动已完成）
2. 运行契约一致性检查（如有前后端交互）：
   - 扫描前端所有 API 调用，提取端点路径
   - 对比后端路由定义
   - 对比 design.md 中定义的契约
   - 发现不匹配时阻断
