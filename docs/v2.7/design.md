...
| 关联需求 | `docs/v2.7/requirements.md` v0.12 |
...
| `service_governance_skill` | 治理模板 | D3 canonical updates + D1/D4 semantic hints | `esb_service` | `ServiceGovernanceProfileUpdater` | D3=`auto_apply`；D1/D4=`draft_apply/suggestion_only` |
...
- PM 导入页：
  ...
  - 上传成功后展示 `execution_id` 与 execution status，不再展示 `history_report` / `esb`
  - 轮询接口从 `profile/extraction-status` 迁移到 `profile/execution-status`
- 系统画像面板：
  - D1-D5 改为读取 `profile_data.<domain>.canonical`
  - 每个域显式展示 `extensions`
  - 增加 `field_sources` / Memory 入口，供 PM/Admin/Expert 查看
- 服务治理页：
  - 上传治理模板
  - 展示 `matched_count`、`unmatched_count`、`updated_system_ids`
  - 支持点击系统查看该次更新的 Memory 记录
- 系统清单导入页：
  - 仅保留单一系统清单导入视图，不再展示“主系统清单 / 子系统清单”双 tab
  - preview 区保持现有校验结果
  - confirm 后新增画像联动结果区，展示 `updated_system_ids` 与 `skipped_items`
...
- `SystemProfileBoardPage`：
  ...
  - 建议新增 Memory Tab，而不是把 Memory 混入事件流
