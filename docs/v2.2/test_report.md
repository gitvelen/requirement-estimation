...
| GWT-REQ-014-01 | REQ-014 | Given v2.2 已部署，When 检查主文档，Then `docs/系统功能说明书.md` 与 `docs/接口文档.md` 均不再把 FUNC-022/`page=ai` 作为可用入口 | TEST-DOC-001 | RUN_OUTPUT | `rg -n "FUNC-022|/reports/ai-effect|page=ai" docs/系统功能说明书.md docs/接口文档.md docs/用户手册.md docs/部署记录.md` -> 命中下线与兼容说明 | ✅ |
...
| GWT-REQ-C005-01 | REQ-C005 | Given v2.2 实现完成，When 检查变更内容，Then 不包含任何数据库 schema 迁移脚本/DDL 变更（仅允许应用层逻辑与前端改造） | TEST-QA-001 | RUN_OUTPUT | `rg -n "CREATE TABLE|ALTER TABLE|DROP TABLE|alembic|op\.add_column|op\.alter_column|op\.drop_column" -S backend tests frontend | head -n 20` -> 空输出 | ✅ |
