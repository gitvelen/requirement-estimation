...
- 备注字段按时间戳追加 AI 摘要并只读展示：保留历史人工备注；触发点覆盖 PM 保存、专家提交、AI 重评估完成。
- 风险控制：所有旧入口保留兼容跳转；上线回滚以版本回退为主（必要时可引入 UI Feature Flag 作为补充）。
...
| D-11 | 备注承载方式 | 任务级 `remark` 字段按行追加 | 保留历史人工备注并追加 AI 摘要（REQ-011） |
...
| REQ-101 | 统一加载/空/错误态 | §5.*（页面统一状态） | UI 验证（Loading/Empty/Error） |
...
- 移除入口：
  ...
  - `/reports/ai-effect` 保留兼容路由，仅做跳转提示（REQ-012）
...
#### 5.5.2 后端强制门禁
- 在 `PUT /api/v1/requirement/features/{task_id}` 等编辑相关接口中：
  - 对 PM（actor_role=manager）禁止写入 `预估人天`（REQ-C001）。
  - 对 PM 的“实质性修改”要求请求显式携带 `confirm=true`，否则拒绝（REQ-C002）。
  - add 操作：PM 新增功能点时，后端不再补默认 `预估人天`（保持为空）。

### 5.6 系统画像：知识导入页 Tab 化 + 文档类型（REQ-008/REQ-C006）
- `frontend/src/pages/SystemProfileImportPage.js`：
  - 页面内部 Tab：代码扫描 / 文档导入。
  - 文档导入 Tab：单文件 Upload + 文档类型下拉（5 种）；不提供批量入口（REQ-C006）。
  - 非 ESB 文档导入：调用 `POST /api/v1/knowledge/imports`，携带 `doc_type`（Form 字段），并写入 metadata。
  - ESB 接口文档：调用 `POST /api/v1/esb/imports`，保留 `mapping_json` 输入。
...
- 后端生成策略：
  - 在 task 上维护 `remark`（text）或 `remark_lines`（list）字段，保留历史人工备注并按时间戳追加新摘要行。
  - 摘要行包含关键词：PM/专家/AI重评估（满足对应 GWT）。
  - 触发点：
...
- 兼容策略：v2.2 保留所有旧 URL replace 重定向，避免死链与返回死循环。
- 风险点：
