...
4. 需求可验收且可追溯：每条需求都有可判定的验收标准（第三方仅凭文字即可判 PASS/FAIL）；并维护追溯链路，能从场景一路追到实现与验证。
...
- **AI 自审（🔴 MUST）**：Phase 03-06 必须执行并落盘 `review_<stage>.md`，具体口径与必填字段统一以 `.aicoding/ai_workflow.md` + `.aicoding/templates/review_template.md` 为准。
- **独立审查（可选）**：当用户消息包含 `@review` 时，进入 Reviewer 模式：只做审查、不修改业务代码；仅允许创建/更新 `docs/<版本号>/review_<stage>.md`（除非用户明确要求“边审边改”）。
- **阶段状态**：`docs/<版本号>/status.md` 是单一真相源；字段约束、枚举与阶段推进规则统一以 `.aicoding/ai_workflow.md` 为准，缺失时用 `.aicoding/templates/status_template.md` 初始化。
